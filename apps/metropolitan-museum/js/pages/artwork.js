...
async function main() {
  ...
  document.title = `${artwork.title} · The Met`;
  ...
  container.innerHTML = `
    <div class="grid md:grid-cols-2 gap-8">
      <div>
        ...
        ${full
          ? `<button type="button" id="deepzoom-btn" class="mt-4 w-full bg-slate-800 text-white py-2.5 rounded font-medium">View in high-res</button>`
          : ""}
      </div>
      <div>
        ...
        <p class="text-gray-700 mt-1">${escapeHtml(a.artist)}${a.artistNationality ? ` <span class="text-gray-500">(${escapeHtml(a.artistNationality)})</span>` : ""}</p>
        ...
        ${a.isPublicDomain ? `<p class="mt-4 text-xs text-gray-500">Public domain (CC0)</p>` : ""}
      </div>
    </div>
  `;
