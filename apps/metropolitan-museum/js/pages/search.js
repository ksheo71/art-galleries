...
async function loadResults(reset) {
  ...
  if (!result.ok) {
    grid.innerHTML = `<div class="col-span-full text-center py-16"><p class="text-gray-600 mb-4">Could not load results.</p><button type="button" class="px-4 py-2 bg-slate-800 text-white rounded" id="retry">Try again</button></div>`;
...
function renderFiltersPanel() {
  ...
  const aggregates = {
    ...
    departments: [...new Set(state.allResults.map((a) => a.department).filter(Boolean))],
  };
  renderFilters(
    container,
