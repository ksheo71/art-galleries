...
export function renderFilters(container, { aggregates, departments, current }, onChange) {
  const activeDepartments = departments.filter((d) =>
    aggregates.departments.includes(d.title)
  );
  ...
  container.innerHTML = `
    <div class="space-y-5">
      ...
      <div class="flex gap-2">
        <button type="button" class="flex-1 bg-slate-800 text-white py-2 rounded text-sm font-medium" id="apply-filters">Apply</button>
