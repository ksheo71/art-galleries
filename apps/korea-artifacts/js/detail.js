// 상세: 단일 유물 + 이미지 갤러리(국가유산청 이미지 API 실시간 호출, CORS 허용)
...
function render(item, images) {
  ...
  detailEl.innerHTML = `
    <div class="grid md:grid-cols-2 gap-8">
      ...
      <div>
        <div class="flex flex-wrap items-center gap-1.5 mb-2">${eraBadge(item)}${typeBadge(item)}${kindBadge(item.kind)}</div>
...
async function init() {
  ...
  if (!item) {
    statusEl.textContent = "해당 유물을 찾을 수 없습니다.";
    return;
  }
