...
function render() {
  ...
  countLabel.textContent = `${items.length} saved object${items.length === 1 ? "" : "s"}`;

  if (items.length === 0) {
    container.innerHTML = `
      <div class="text-center py-16 max-w-md mx-auto">
        <p class="text-gray-700 text-lg">You haven't saved anything yet.</p>
        <p class="text-gray-500 text-sm mt-2">Tap ♥ on any object to save it here.</p>
        <a href="./index.html" class="inline-block mt-6 px-4 py-2 bg-amber-800 text-white rounded">Back to home</a>
      </div>
    `;
    return;
  }
