...
async function loadResults(reset) {
  ...
  const result = await api.search({ q: state.q, page: state.page, limit: 12 });
  if (!result.ok) {
    grid.innerHTML = `<div class="col-span-full text-center py-16"><p class="text-gray-600 mb-4">Could not load results.</p><button type="button" class="px-4 py-2 bg-blue-900 text-white rounded" id="retry">Try again</button></div>`;
...
function applyClientFilters(items) {
  return items.filter((a) => {
    ...
    if (state.filters.department && a.department !== state.filters.department) return false;
...
function renderFiltersPanel() {
  ...
  const aggregates = {
    ...
    departments: [...new Set(state.allResults.map((a) => a.department).filter(Boolean))].sort(),
  };
  renderFilters(
    container,
    { aggregates, departments: state.departments, current: state.filters },
    onFiltersChange
  );
}
...
function updateCountLabel() {
  ...
  if (state.pagination.total > state.allResults.length) {
    label.textContent = `Showing ${filtered.length} of ~${state.pagination.total} objects — "Load more" for a wider filter pool.`;
  } else {
    label.textContent = `${filtered.length} object${filtered.length === 1 ? "" : "s"}`;
  }
}
