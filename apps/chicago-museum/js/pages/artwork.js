...
import { openDeepZoom } from "../iiif.js";
import * as favorites from "../favorites.js";
import { escapeHtml, sanitizeDescription } from "../util.js";
...
async function main() {
  ...
  document.title = `${artwork.title} · Art Gallery · Chicago`;
...
function renderDetail(a) {
  ...
  const imgUrl = api.iiifThumbUrl(a.imageId, 843);
  ...
  container.innerHTML = `
    <div class="grid md:grid-cols-2 gap-8">
      <div>
        ${imgUrl
          ? `<img src="${imgUrl}" alt="${escapeHtml(a.title)} by ${escapeHtml(a.artist)}" class="w-full rounded shadow-sm bg-gray-100"/>`
        ...
        ${imgUrl ? `<button type="button" id="deepzoom-btn" class="mt-4 w-full bg-black text-white py-2.5 rounded font-medium">View in high-res</button>` : ""}
      </div>
      <div>
        ...
        <dl class="mt-6 space-y-2 text-sm">
          ...
          ${fieldRow("Department", a.departmentTitle)}
    ...
    ${a.description
      ? `<div class="mt-10 max-w-3xl">
           ...
           <div class="text-gray-800 leading-relaxed space-y-3" id="description-body"></div>
         </div>`
      : ""}
  `;
  ...
  if (a.description) {
    ...
    body.innerHTML = sanitizeDescription(a.description);
  }

  const deepBtn = document.getElementById("deepzoom-btn");
  if (deepBtn) deepBtn.addEventListener("click", () => openDeepZoom(a.id));
...
async function loadRelated(a) {
  ...
  const r = await api.fetchRelated({
    artistId: a.artistId,
    departmentTitle: a.departmentTitle,
    excludeId: a.id,
    limit: 6,
  });
