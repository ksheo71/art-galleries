...
export function renderFilters(container, { aggregates, departments, current }, onChange) {
  // Cleveland: `departments` arrives empty (no dedicated endpoint), so build
  // the dropdown from aggregated result values only.
  ...
  container.innerHTML = `
    <div class="space-y-5">
      <div>
        <h4 class="text-xs font-bold uppercase tracking-wide mb-2 text-gray-500">Artist</h4>
      ...
      <div>
        <h4 class="text-xs font-bold uppercase tracking-wide mb-2 text-gray-500">Department</h4>
        <select name="f-dept" class="w-full border rounded px-2 py-1 text-sm" aria-label="Department">
      ...
      <div class="flex gap-2">
        <button type="button" class="flex-1 bg-emerald-800 text-white py-2 rounded text-sm font-medium" id="apply-filters">Apply</button>
