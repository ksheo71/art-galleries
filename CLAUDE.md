...
- 시작은 The Art Institute of Chicago 로 하지만 점점 더 추가해 나갈 계획. 현재 구현된 앱: chicago-museum, metropolitan-museum, cleveland-museum, vna-east-museum, yale-museum, harvard-museum, korea-heritage, korea-artifacts, emuseum, exhibitions.
- `exhibitions` 는 박물관 소장품이 아니라 **전국 미술관·박물관의 진행중·예정·지난 전시**를 장르별로 보여주는 앱이다. 소스는 **둘**: ① KCISA 문화포털 공연·전시 API(`api.kcisa.kr/openapi/API_CCA_145/request`, `EXHIBITIONS_API_KEY`, 국립기관 위주) ② 서울 열린데이터광장 문화행사(`openapi.seoul.go.kr:8088/{KEY}/json/culturalEventInfo`, `SEOUL_API_KEY`, CODENAME=전시 필터 → 서울 시립·구립·문화재단 venue 보강). 두 소스를 `title|institution` 으로 중복제거 병합. 상태는 진행중/예정/지난(최근 `PAST_DAYS`=180일 종료분), 기본 뷰는 "현재"(진행중+예정)라 지난은 칩으로만. **인사동 등 상업 인디 갤러리는 어떤 공공 API에도 없어 미포함**(네오룩·김달진 링크아웃으로 안내). **빌드타임이 아니라 매일 야간 배치**(맥미니 launchd `com.art-galleries.exhibitions`, 새벽 04:10)로 harvest 를 돌려 `data/exhibitions.json` 을 갱신한다(키는 운영 `.env` 의 `EXHIBITIONS_API_KEY`, 브라우저 비노출; 프록시 불필요). 주의: ① API_CCA_145 는 **http/https 모두 OK 지만 전용 키**가 필요(MPKreli/emuseum 키와 다름), ② `GENRE` 필드는 미술 장르가 아니라 상태값(과거/현재/예정전시)이라 **장르는 제목·설명 키워드로 분류**, ③ 기간은 `PERIOD`/`EVENT_PERIOD` 자유텍스트, ④ 포스터는 재호스팅하지 않고 카드가 **원문(URL)으로 링크아웃**(저작권). `data/exhibitions.json` 은 `.gitignore`(매일 생성, deploy `git reset` 가 안 건드리게).
